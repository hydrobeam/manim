...
from manim.utils.space_ops import angle_of_vector, rotation_matrix_transpose
...
class OpenGLMobject:
    ...
    def add(self, *mobjects):
        if self in mobjects:
            raise Exception("OpenGLMobject cannot contain self")
    ...
    def scale(
        self, scale_factor, min_scale_factor=1e-8, about_point=None, about_edge=ORIGIN
    ):
        """
    ...
    def put_start_and_end_on(self, start, end):
        ...
        self.rotate(
            np.arctan2(curr_vect[2], np.linalg.norm(curr_vect[:2]))
            - np.arctan2(target_vect[2], np.linalg.norm(target_vect[:2])),
